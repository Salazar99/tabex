...
fn test_and() {
    let a = prop("a");
    let b = prop("b");
    let expected: Node = Node::from_operands(vec![a.clone(), b.clone()]);
...
fn test_or() {
    let a = prop("a");
    let b = prop("b");
    let expected1: Node = Node::from_operands(vec![a.clone()]);
    let expected2: Node = Node::from_operands(vec![b.clone()]);
    make_test_decompose(
        vec![Formula::or(vec![a, b])],
...
fn test_imply() {
    ...
    let expected1: Node = Node::from_operands(vec![*not_left.clone()]);
    let expected_optimization: Node = Node::from_operands(vec![a.clone(), b.clone()]);
    let expected_non_optimization: Node = Node::from_operands(vec![b.clone()]);
    make_test_decompose(
        vec![imply.clone()],
...
fn test_globally() {
    ...
    make_test_decompose(
        vec![Formula::g(Interval { lower: 0, upper: 5 }, None, a)],
        vec![expected1],
        None,
    );
}
...
fn test_finally() {
    let a = prop("a");
    let expected1: Node = Node::from_operands(vec![a.clone()]);
    let expected2: Node = Node::from_operands(vec![Formula::o(Formula::f(
        Interval { lower: 0, upper: 5 },
        None,
        a.clone(),
    ))]);
    make_test_decompose(
        vec![Formula::f(Interval { lower: 0, upper: 5 }, None, a.clone())],
        vec![expected1, expected2],
        None,
    );
}
...
fn test_finally_end() {
    ...
    let expected1: Node = Node::from_operands(vec![a.clone()]);
    make_test_decompose(
        vec![Formula::f(Interval { lower: 0, upper: 0 }, None, a.clone())],
        vec![expected1],
        None,
    );
}
...
fn test_until() {
    let a = prop("a");
    let b = prop("b");
    let expected1: Node = Node::from_operands(vec![b.clone()]);
...
fn test_release() {
    let a = prop("a");
    let b = prop("b");
    let expected1: Node = Node::from_operands(vec![a.clone(), b.clone()]);
    let expected2: Node = Node::from_operands(vec![
        b.clone(),
        Formula::o(Formula::r(
            Interval { lower: 0, upper: 5 },
...
fn test_jump_only_prop() {
    let a = prop("a");
    let b = prop("b");
    let to_decompose = Node::from_operands(vec![a.clone(), b.clone()]);
...
fn test_jump_step_interval_end() {
    ...
    assert_eq!(vec.len(), 1);
    let node = &vec[0];
    let expected = Node::from_operands(vec![Formula::f(
        Interval { lower: 0, upper: 8 },
        None,
        Formula::not(a.clone()),
    )]);
...
fn test_jump_step_closure() {
    let a = prop("a");
    let mut to_decompose = Node::from_operands(vec![Formula::o(Formula::g(
        Interval { lower: 0, upper: 5 },
        None,
        Formula::f(Interval { lower: 0, upper: 2 }, None, a.clone()),
    ))]);
    ...
    assert_eq!(vec.len(), 1);
    let node = &vec[0];
    let expected = Node::from_operands(vec![Formula::g(
        Interval { lower: 0, upper: 5 },
        None,
        Formula::f(Interval { lower: 0, upper: 2 }, None, a.clone()),
    )]);
...
fn test_jump_end() {
    let a = prop("a");
    let mut to_decompose = Node::from_operands(vec![Formula::o(Formula::f(
        Interval {
            lower: 20,
            upper: 50,
        },
